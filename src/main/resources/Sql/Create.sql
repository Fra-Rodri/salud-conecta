...
CREATE TABLE clinica.usuario (
	...
	CONSTRAINT usuario_negocio_id_fk FOREIGN KEY (negocio_id) REFERENCES clinica.negocio(id) -- Si se borra un negocio no se borra el usuario
);
...
CREATE TABLE clinica.informe (
    ...
    CONSTRAINT informe_usuario_id_fk FOREIGN KEY (usuario_id) REFERENCES clinica.usuario(id), -- Si se elimina un usuario (profesional), no se eliminan los informes
    CONSTRAINT informe_paciente_id_fk FOREIGN KEY (paciente_id) REFERENCES clinica.paciente(id) -- Si se elimina un paciente, no se eliminan sus informes
